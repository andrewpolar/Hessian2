...
public:
	...
	double GetJacobianElementsAndUrysohn(const std::unique_ptr<double[]>& inputs, std::vector<double>& derivatives, std::vector<int>& indexes) {
		for (int k = 0; k < (int)derivatives.size(); ++k) derivatives[k] = 0.0;
		for (int k = 0; k < (int)indexes.size(); ++k) indexes[k] = 0;
		...
		for (int i = 0; i < (int)_model.size(); ++i) {
			f += GetJacobianElementsAndFunction(i, inputs[i], leftOffset, rightOffset, leftIndex, rightIndex);
			derivatives[counterD++] = rightOffset;
			derivatives[counterD++] = leftOffset;
			indexes[counterI++] = leftIndex + nPoints * i;
			indexes[counterI++] = rightIndex + nPoints * i;
		}
		return f;
	}
...
private:
	...
	double GetJacobianElementsAndFunction(int k, double x, double& leftOffset, double& rightOffset, int& leftIndex, int& rightIndex) {
		if (x <= _xmin[k]) {
			int index = 0;
			leftOffset = 0.0;
			rightOffset = 1.0;
